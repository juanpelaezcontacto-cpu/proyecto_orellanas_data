...
bool sht1_disponible = false;
bool sht2_disponible = false;
...
const unsigned long tiempo_min_apagado = 420000; // 7 minutos de proteccion del compresor
...
void setup() {
  ...
  unsigned long inicio_wifi = millis();
  while (WiFi.status() != WL_CONNECTED && millis() - inicio_wifi < WIFI_TIMEOUT_MS) {
    delay(500);
    Serial.print(".");
  }
  if (WiFi.status() == WL_CONNECTED) {
    ...
    Serial.println("\nWi-Fi no conectado. El control local continua activo.");
  }
  ...
  // ---------------- SHT40 1 ----------------
  sht1_disponible = sht1.begin(Wire, 0x44, SCL_P, SDA_P);
  if (!sht1_disponible) {
    Serial.println("Error iniciando SHT40 1");
  }

  // ---------------- SHT40 2 ----------------
  sht2_disponible = sht2.begin(Wire1, 0x44, SCL_S, SDA_S);
  if (!sht2_disponible) {
    Serial.println("Error iniciando SHT40 2");
  }
  if (sht1_disponible) sht1.setPrecision(REPEATABILITY_HIGH);
  if (sht2_disponible) sht2.setPrecision(REPEATABILITY_HIGH);
  if (sht1_disponible && sht2_disponible) {
...
void actualizarCicloCompresor() {
  // Si está encendido, NO está disponible para volver a arrancar. Su tiempo restante es 0.
  if (estado_compresor == 1) {
    compresor_disponible = 0; 
    tiempo_restante_ciclo = 0;
  } 
  // Si está apagado, calculamos la restricción por presiones
  else {
    unsigned long tiempo_transcurrido = millis() - tiempo_ultimo_apagado;
    
    if (tiempo_transcurrido >= tiempo_min_apagado) {
      compresor_disponible = 1; // El motor ha descansado lo suficiente. Es SEGURO encender.
      tiempo_restante_ciclo = 0;
    } else {
      compresor_disponible = 0; // Sigue bloqueado por seguridad térmica/presión
...
void calcularTemperaturaAmbiente(float t2, float t_inf) {
  ...
  // Guardar en el nuevo arreglo independiente
...
// =========================================================================
// MAQUINA DE ESTADOS HÍBRIDA PARA VENTILADORES (No bloqueante)
// =========================================================================
void gestionarVentiladoresInteligentes() {
  ...
  // CASO C: Periodo de reposo térmico (Compresor apagado y ya pasó el post-enfriamiento)
  // Evaluamos ciclos temporales cortos solo para romper bolsas de CO2 estancadas
  unsigned long tiempo_desde_ultimo_ciclo_recirculacion = millis() - cronometro_recirculacion;
